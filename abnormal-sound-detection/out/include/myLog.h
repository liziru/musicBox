...
myLog::myLog():rootCategory(log4cpp::Category::getRoot().getInstance("rootCategory"))
{
	...
	//log文件会一直记录
	log4cpp::FileAppender*fileAppender = new log4cpp::FileAppender("fileAppender","/tmp/allLog.log");
...
//			"rollingfileAppender","/tmp/logrolling.log",5*1024,5);
...
#define catMsg(msg) string("{fileName:")\
	.append(__FILE__).append(" functionName:")\
	.append(__func__).append(" lineNumber:")\
	.append(int2string(__LINE__)).append("} ").append(string(msg)).c_str()
//单例类，只有一个对象
...
#define logFatal(msg) log->fatal(msg)
